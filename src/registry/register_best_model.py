...
def _registry_context(config: dict) -> tuple[str, MlflowClient]:
    ...
    return tracking_uri, MlflowClient(tracking_uri=tracking_uri, registry_uri=tracking_uri)
...
def _candidate_threshold_result(config: dict) -> dict[str, Any]:
    ...
    accuracy_threshold = float(get_config_value(config, 'continuous_improvement.accuracy_threshold', 0.0))
    macro_f1_threshold = float(get_config_value(config, 'continuous_improvement.macro_f1_threshold', 0.0))
    passed = (
...
def register_candidate_model(run_id: str | None = None) -> dict[str, Any]:
    ...
    status = {
        ...
        'previous_champion': _current_champion(client, model_name, champion_alias, metric_name),
...
def promote_candidate_model() -> dict[str, Any]:
    ...
    if not validation['passed']:
        ...
        store_pipeline_artifact('registry_status', status, config=config, run_id=str(resolved_run_id))
        LOGGER.info('Candidate rejected by validation gate: %s', status)
        return status

    model_name = get_config_value(config, 'registry.model_name', 'galaxy_morphology_classifier')
    champion_alias = get_config_value(config, 'registry.champion_alias', 'champion')
    metric_name = candidate.get('metric_name') or get_config_value(config, 'registry.comparison_metric', 'macro_f1')
    ...
    store_pipeline_artifact('registry_status', status, config=config, run_id=str(resolved_run_id), recorded_at=status['promoted_at'])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('command', nargs='?', choices=['register-candidate', 'promote-candidate', 'register-and-promote'], default='register-and-promote')
