...
def generate_report() -> dict:
    ...
    processed_v1_summary = load_pipeline_artifact(
        "processed_v1_summary", config=config, default={}
    )
    processed_final_summary = load_pipeline_artifact(
        "processed_final_summary", config=config, default={}
    )
    train_metrics = load_pipeline_artifact("train_metrics", config=config, default={})
    validation_metrics = load_pipeline_artifact(
        "validation_metrics", config=config, default={}
    )
    ...
    registry_status = load_pipeline_artifact(
        "registry_status", config=config, default={}
    )
...
def parse_args() -> argparse.Namespace:
    return argparse.ArgumentParser(
        description="Generate the latest markdown and HTML report."
    ).parse_args()
