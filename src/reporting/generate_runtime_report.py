...
def generate_runtime_report() -> dict[str, Any]:
    ...
    runtime_summary = load_pipeline_artifact('pipeline_runtime_summary', config=config, default={}) or {}
    registry_status = load_pipeline_artifact('registry_status', config=config, default={}) or {}
    ...
    title = get_config_value(
        ...
        get_config_value(config, 'reporting.report_title', 'Galaxy Morphology Training and Monitoring Report'),
    )
    ...
    _append_section(
        ...
        [
            ...
            _metric_line('Validation precision macro', validation_metrics, 'precision_macro'),
    ...
    _append_section(
        ...
        [
            ...
            _metric_line(f"Candidate metric ({_value(candidate, 'metric_name')})", candidate, 'metric_value'),
            _metric_line('Previous champion version', previous if isinstance(previous, dict) else {}, 'version'),
            _metric_line(
            ...
            _metric_line('Current champion version', registry_data, 'current_champion_version'),
    ...
    _append_section(
        ...
        [
            ...
            _metric_line('Latest-model prediction count', live_metrics, 'prediction_count'),
            _metric_line('Feedback count', live_metrics, 'feedback_count'),
            _metric_line('Assumed correct without correction feedback', live_metrics, 'assumed_correct_count'),
    ...
    _append_section(
        ...
        'Training runtime',
        [
            _metric_line('Training duration seconds', runtime_summary, 'train_duration_seconds'),
    ...
    html_sections = [
        _html_section(
            'Validation metrics',
            [
                _html_metric_item('Validation accuracy', validation_metrics, 'accuracy'),
                _html_metric_item('Validation macro F1', validation_metrics, 'macro_f1'),
                _html_metric_item('Validation precision macro', validation_metrics, 'precision_macro'),
                _html_metric_item('Validation recall macro', validation_metrics, 'recall_macro'),
            ],
        ),
        _html_section(
            'Offline metrics',
        ...
        _html_section(
            ...
            [
                ...
                _html_metric_item(f"Candidate metric ({_value(candidate, 'metric_name')})", candidate, 'metric_value'),
                _html_metric_item('Previous champion version', previous if isinstance(previous, dict) else {}, 'version'),
                _html_metric_item(
                    f"Previous champion metric ({_value(previous, 'metric_name') if isinstance(previous, dict) else 'n/a'})",
                    previous if isinstance(previous, dict) else {},
                    'metric_value',
                ),
                _html_metric_item('Champion updated', registry_data, 'champion_updated'),
                _html_metric_item('Current champion version', registry_data, 'current_champion_version'),
                _html_metric_item('Serving model URI', registry_data, 'serving_model_uri'),
        ...
        _html_section(
            'Continuous improvement',
            [
                _html_metric_item('Latest model version', live_metrics, 'latest_model_version'),
                _html_metric_item('Latest-model prediction count', live_metrics, 'prediction_count'),
                _html_metric_item('Feedback count', live_metrics, 'feedback_count'),
                _html_metric_item('Assumed correct without correction feedback', live_metrics, 'assumed_correct_count'),
    ...
    if raw_counts:
        html_sections.append(f'<h2>Raw data counts</h2><pre>{html.escape(str(raw_counts))}</pre>')
    html_sections.append(
        _html_section(
            'Training runtime',
            [
                _html_metric_item('Training duration seconds', runtime_summary, 'train_duration_seconds'),
                _html_metric_item('Epochs completed', runtime_summary, 'epochs_completed'),
            ],
        )
    )
...
def parse_args() -> argparse.Namespace:
    return argparse.ArgumentParser(description='Generate the latest Airflow runtime report.').parse_args()
