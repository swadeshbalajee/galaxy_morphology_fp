...
def log_numeric_metrics(prefix: str, metrics: dict) -> None:
    numeric_metrics = {
...
def evaluate_live_feedback(
    predictions_db_path: str | Path | None = None,
) -> dict:  # noqa: ARG001
    config = load_config()
    initialize_database()
    metrics = {
...
def main() -> None:
    ...
    with mlflow.start_run(run_name="galaxy_evaluation_run"):
        offline_metrics = evaluate_offline()
        live_metrics = evaluate_live_feedback()
        log_numeric_metrics("test", offline_metrics)
        log_numeric_metrics("live", live_metrics)
        mlflow.log_dict(offline_metrics, "test_metrics.json")
        mlflow.log_dict(live_metrics, "live_metrics.json")
    LOGGER.info(
        "Evaluation complete. offline=%s live=%s", offline_metrics, live_metrics
    )
