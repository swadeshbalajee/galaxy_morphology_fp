...
def load_feedback_samples(
    config: dict, class_to_idx: dict[str, int]
) -> list[tuple[Path, int]]:
    ...
    with manifest_path.open("r", newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            if label not in class_to_idx:
                LOGGER.warning(
                    "Skipping feedback sample with unknown label=%s path=%s",
                    label,
                    image_path,
                )
                continue
            ...
            if not resolved_path.exists():
                LOGGER.warning(
                    "Skipping feedback sample with missing image=%s", resolved_path
                )
                continue
...
def make_dataloaders(train_dir: str, val_dir: str, config: dict):
    ...
    train_data = (
        ConcatDataset(
            [base_train_data, FeedbackManifestDataset(feedback_samples, transform)]
        )
        if feedback_samples
    ...
    if feedback_samples:
        LOGGER.info(
            "Loaded %s materialized feedback samples into the training dataset",
...
def evaluate_loader(model, loader, device):
    ...
    precision_weighted, recall_weighted, weighted_f1, _ = (
        precision_recall_fscore_support(
...
def train(train_dir: str, val_dir: str, _test_dir: str | None, export_dir: str):
    ...
    (
        class_names,
        feedback_train_samples,
        train_data,
        val_data,
        train_loader,
        val_loader,
    ) = make_dataloaders(train_dir, val_dir, config)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = build_model(
        len(class_names),
        get_config_value(config, "training.pretrained_backbone", "resnet18"),
    )
    model.to(device)
    criterion = nn.CrossEntropyLoss()
    ...
    with mlflow.start_run(run_name="galaxy_training_run") as run:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_idx, (images, labels) in enumerate(train_loader, start=1):
                ...
                if batch_idx % 10 == 0 or batch_idx == len(train_loader):
                    LOGGER.info(
                        "epoch=%s batch=%s/%s running_train_loss=%.4f samples_seen=%s",
                        epoch,
                        batch_idx,
                        len(train_loader),
            ...
            epoch_result = {
                ...
                "samples_per_second": round(
                    len(train_loader.dataset) / max(epoch_duration, 1e-6), 3
                ),
            }
            ...
            mlflow.log_metrics(
                {
                    ...
                    "val_macro_f1": val_metrics["macro_f1"],
                    "val_weighted_f1": val_metrics["weighted_f1"],
                    "epoch_duration_seconds": epoch_result["epoch_duration_seconds"],
        ...
        validation_confusion_df.to_csv(confusion_path)
        ...
        store_pipeline_artifact(
            "train_metrics", train_metrics, config=config, run_id=run.info.run_id
        )
        store_pipeline_artifact(
            "validation_metrics",
            validation_summary,
        ...
        LOGGER.info(
            "Training finished. train_metrics=%s validation_metrics=%s",
            train_metrics,
            validation_summary,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a galaxy morphology classifier."
    )
...
if __name__ == "__main__":
    ...
    train(
        args.train_dir or processed_root / "train",
        args.val_dir or processed_root / "val",
        args.test_dir or processed_root / "test",
        args.export_dir or resolve_path(cfg, "paths.models_dir"),
    )
