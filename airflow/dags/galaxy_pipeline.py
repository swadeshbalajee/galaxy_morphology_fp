...
DEFAULT_ARGS = {
    ...
    'retry_delay': timedelta(minutes=int(_required_config('orchestration.airflow_retry_delay_minutes'))),
}
...
def _run_command(command: list[str], use_training_venv: bool = False) -> None:
    if command[:2] == ['git', 'add'] or command[:2] == ['git', 'commit'] or command[:2] == ['git', 'push']:
        raise RuntimeError('Airflow DAG tasks must not run git add, git commit, or git push.')
...
def _deployment_metadata() -> dict[str, str | None]:
    return {
        'git_commit_sha': os.getenv('DEPLOYMENT_GIT_COMMIT_SHA') or os.getenv('GIT_COMMIT_SHA'),
...
def _latest_mlflow_run_id() -> str:
    ...
    if not run_id:
        raise RuntimeError('Could not determine MLflow run_id from train_metrics for provenance logging.')
    return str(run_id)
...
def _candidate_metrics() -> dict:
    ...
    validation_metrics = load_pipeline_artifact('validation_metrics', config=CONFIG, default={}) or {}
    return {
...
def validate_candidate_thresholds() -> dict:
    ...
    accuracy_threshold = float(_required_config('continuous_improvement.accuracy_threshold'))
    macro_f1_threshold = float(_required_config('continuous_improvement.macro_f1_threshold'))
    passed = (
...
def sync_feedback_training_snapshot() -> dict:
    ...
    min_new_feedback = int(_required_config('continuous_improvement.min_new_feedback_samples'))
...
def inspect_runtime_state() -> dict:
    ...
    macro_f1_threshold = float(_required_config('continuous_improvement.macro_f1_threshold'))
    min_new_feedback = int(_required_config('continuous_improvement.min_new_feedback_samples'))
    min_live_predictions = int(_required_config('continuous_improvement.min_live_prediction_samples'))
    ...
    if isinstance(live_accuracy, (int, float)) and live_prediction_count >= min_live_predictions and live_accuracy < accuracy_threshold:
        model_degraded = True
    elif isinstance(live_macro_f1, (int, float)) and live_prediction_count >= min_live_predictions and live_macro_f1 < macro_f1_threshold:
        model_degraded = True
...
def save_dvc_provenance(**context) -> dict:
    ...
    if not DVC_LOCK_PATH.exists():
        raise FileNotFoundError(f'Cannot save provenance because {DVC_LOCK_PATH} does not exist.')

    shutil.copy2(DVC_LOCK_PATH, copied_lock_path)
    feedback_summary = load_pipeline_artifact('feedback_training_summary', config=CONFIG, default={}) or {}
    control_state = _control_plane_state()
    ...
    provenance = {
        ...
        'feedback_snapshot_id': control_state.get('last_feedback_snapshot_at') or control_state.get('last_feedback_snapshot_count'),
        'feedback_snapshot_path': feedback_summary.get('manifest_path') or str(FEEDBACK_TRAINING_MANIFEST_PATH),
...
def log_dvc_provenance_to_mlflow(**context) -> None:
    ...
    tracking_uri = os.getenv('MLFLOW_TRACKING_URI') or get_config_value(CONFIG, 'services.mlflow_url', 'http://mlflow:5000')
    ...
    with mlflow.start_run(run_id=mlflow_run_id):
        ...
        mlflow.log_artifact(str(copied_lock_path), artifact_path=f'provenance/{safe_run_id}')
        mlflow.log_artifact(str(provenance_path), artifact_path=f'provenance/{safe_run_id}')
    LOGGER.info('Logged DVC provenance artifacts to MLflow run %s', mlflow_run_id)


def register_candidate_model() -> None:
    _run_command([TRAINING_PYTHON, '-m', 'src.registry.register_best_model', 'register-candidate'], use_training_venv=True)
...
def promote_candidate_model() -> None:
    _run_command([TRAINING_PYTHON, '-m', 'src.registry.register_best_model', 'promote-candidate'], use_training_venv=True)
...
def generate_runtime_report() -> None:
    _run_command([TRAINING_PYTHON, '-m', 'src.reporting.generate_runtime_report'], use_training_venv=True)
...
def send_latest_report() -> None:
    ...
    report_text = RUNTIME_REPORT_MD_PATH.read_text(encoding='utf-8') if RUNTIME_REPORT_MD_PATH.exists() else 'No runtime report generated yet.'
    report_html = RUNTIME_REPORT_HTML_PATH.read_text(encoding='utf-8') if RUNTIME_REPORT_HTML_PATH.exists() else None
    ...
    attachments = [str(path) for path in (RUNTIME_REPORT_MD_PATH, RUNTIME_REPORT_HTML_PATH) if path.exists()]
...
with DAG(
    ...
    dvc_push = PythonOperator(task_id='push_dvc_artifacts', python_callable=push_dvc_artifacts)
    save_provenance = PythonOperator(task_id='save_dvc_provenance', python_callable=save_dvc_provenance)
    log_provenance = PythonOperator(task_id='log_dvc_provenance_to_mlflow', python_callable=log_dvc_provenance_to_mlflow)
    register_model = PythonOperator(task_id='register_candidate_model', python_callable=register_candidate_model)
    validate_candidate = BranchPythonOperator(task_id='validate_candidate_model', python_callable=branch_candidate_validation)
    promote_model = PythonOperator(task_id='promote_candidate_model', python_callable=promote_candidate_model)
    reject_model = PythonOperator(task_id='reject_candidate_model', python_callable=reject_candidate_model)
    ...
    decide >> run_pipeline >> dvc_push >> save_provenance >> log_provenance >> register_model >> validate_candidate
    validate_candidate >> promote_model >> reload_service >> runtime_report >> send_email >> finish
