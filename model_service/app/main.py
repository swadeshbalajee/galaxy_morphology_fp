...
MODEL_REQUESTS = Counter(
    "galaxy_model_requests_total", "Total model inference calls.", ["status"]
)
MODEL_LATENCY = Histogram(
    "galaxy_model_latency_seconds", "Model inference latency in seconds."
)
MODEL_READY = Gauge("galaxy_model_ready", "Model readiness state.")
MODEL_INPUT_BRIGHTNESS_ZSCORE = Gauge(
    "galaxy_input_brightness_zscore", "Brightness drift z-score for latest request."
)
MODEL_LAST_LATENCY_MS = Gauge(
    "galaxy_model_last_latency_ms", "Last model latency in milliseconds."
)
...
def ready():
    ...
    raise HTTPException(
        status_code=503,
        detail="Model artifact not available. Train/export the model first.",
    )
