...
CONFIG = load_config()
API_URL = os.getenv("API_URL") or get_config_value(
    CONFIG, "services.api_url", "http://api:8000"
)
REPORT_PATH = get_config_value(
    CONFIG, "paths.latest_report_md_path", "artifacts/reports/latest_report.md"
)
...
def show_csv_errors(errors: list[dict]):
    st.error(
        "The uploaded correction CSV has validation errors. Fix these rows and upload again."
    )
...
st.caption(
    "Run single-image or ZIP batch inference, download filtered prediction history, and upload correction CSV files backed by Postgres state."
)
...
with st.sidebar:
    ...
    st.write(
        "DVC pipeline → Airflow control plane → MLflow + Prometheus + Grafana + Loki"
    )
...
with single_tab:
    uploaded = st.file_uploader(
        "Upload a galaxy image", type=["png", "jpg", "jpeg", "bmp", "webp"]
    )
    ...
    with right:
        if uploaded is not None and st.button("Predict Morphology", type="primary"):
            ...
            if response.ok:
                ...
                LOGGER.info(
                    "Prediction displayed prediction_id=%s", result["prediction_id"]
                )
            else:
    ...
    if "last_prediction_id" in st.session_state:
        ...
        notes = st.text_area(
            "Optional notes", placeholder="Was the image noisy? Was it ambiguous?"
        )
        if st.button("Submit Feedback"):
            ...
            if response.ok:
                LOGGER.info(
                    "Feedback submitted for prediction_id=%s", payload["prediction_id"]
                )
...
with batch_tab:
    ...
    zip_file = st.file_uploader(
        "Upload ZIP archive of images", type=["zip"], key="zip_uploader"
    )
    if zip_file is not None and st.button("Run batch prediction", type="primary"):
        with st.spinner("Running batch inference..."):
            response = requests.post(
                ...
                files={
                    "zip_file": (zip_file.name, zip_file.getvalue(), "application/zip")
                },
                timeout=600,
            )
        if response.ok:
            payload = response.json()
            st.success(
                f"Batch stored in Postgres. Batch id: `{payload['batch_id']}` | Images: `{payload['total_images']}`"
            )
...
with feedback_tab:
    ...
    st.caption(
        "Use the exported recent-predictions CSV template, fill only the rows that need correction, and set `corrected_label`."
    )
    feedback_csv = st.file_uploader(
        "Upload correction CSV", type=["csv"], key="feedback_csv_uploader"
    )
    if feedback_csv is not None and st.button(
        "Validate and upload CSV", type="primary"
    ):
        response = requests.post(
        ...
        if response.ok:
            st.success(
                f"Correction CSV uploaded successfully. Upload id: `{payload['upload_id']}` | Rows stored: `{payload['row_count']}`"
            )
        else:
...
with recent_tab:
    ...
    try:
        response = requests.get(
            f"{API_URL}/recent-predictions", params=params, timeout=30
        )
        if response.ok:
            payload = response.json()
            ...
            st.warning(
                f"Unable to fetch recent predictions: {response_error_message(response)}"
            )
    except Exception as exc:  # noqa: BLE001
    ...
    if recent:
        export_response = requests.get(
            f"{API_URL}/recent-predictions/export", params=params, timeout=60
        )
        if export_response.ok:
            ...
            st.warning(
                f"Unable to export recent predictions: {response_error_message(export_response)}"
            )
...
try:
    ...
    if os.path.exists(report_path):
        ...
        st.info(
            "No report generated yet. Run the DVC pipeline or Airflow controller first."
        )
except Exception as exc:  # noqa: BLE001
