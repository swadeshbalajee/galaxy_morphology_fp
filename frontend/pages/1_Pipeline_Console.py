...
API_URL = os.getenv("API_URL") or get_config_value(
    CONFIG, "services.api_url", "http://localhost:8000"
)
AIRFLOW_URL = os.getenv("AIRFLOW_URL") or get_config_value(
    CONFIG, "services.airflow_url", "http://localhost:8080"
)
MLFLOW_URL = os.getenv("MLFLOW_URL") or get_config_value(
    CONFIG, "services.mlflow_url", "http://localhost:5000"
)
PROMETHEUS_URL = os.getenv("PROMETHEUS_URL") or get_config_value(
    CONFIG, "services.prometheus_url", "http://localhost:9090"
)
GRAFANA_URL = os.getenv("GRAFANA_URL") or get_config_value(
    CONFIG, "services.grafana_url", "http://localhost:3000"
)
LOKI_URL = os.getenv("LOKI_URL") or get_config_value(
    CONFIG, "services.loki_url", "http://localhost:3100"
)
PIPELINE_EXPORTER_URL = os.getenv(
    "PIPELINE_EXPORTER_URL", "http://pipeline-exporter:8010"
)
...
AIRFLOW_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", AIRFLOW_URL)
MLFLOW_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", MLFLOW_URL)
PROMETHEUS_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", PROMETHEUS_URL)
GRAFANA_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", GRAFANA_URL)
LOKI_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", LOKI_URL)
API_URL = re.sub(r"(?<=http://)[^:/\s]+(?=:\d+)", "localhost", API_URL)
