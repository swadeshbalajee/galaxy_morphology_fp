...
ALLOWED_SUFFIXES = {suffix.lower() for suffix in get_config_value(CONFIG, 'data.allowed_suffixes', ['.jpg', '.jpeg', '.png', '.bmp', '.webp'])}
...
async def startup_event():
    ...
    LOGGER.info('API gateway started. max_upload_mb=%s max_zip_upload_mb=%s', MAX_UPLOAD_MB, MAX_ZIP_UPLOAD_MB)
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        batch_id = _get_store().create_batch(source_filename=file.filename, source_type='single_image', total_files=1)
...
async def predict_batch(zip_file: UploadFile = File(...)):
    ...
    batch_id = _get_store().create_batch(source_filename=zip_file.filename, source_type='zip_archive', total_files=len(image_members))
    ...
    LOGGER.info('Batch prediction completed archive=%s rows=%s batch_id=%s', zip_file.filename, len(items), batch_id)
...
async def recent_predictions(
    ...
    try:
        ...
        LOGGER.info('Recent predictions requested limit=%s returned=%s start_date=%s end_date=%s', limit, len(items), start_date, end_date)
...
async def export_recent_predictions(
    ...
    try:
        content = _get_store().export_recent_predictions_csv(limit=limit, start_date=start_date, end_date=end_date)
