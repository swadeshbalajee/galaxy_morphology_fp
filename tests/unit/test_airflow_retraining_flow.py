...
def galaxy_pipeline(monkeypatch):
    ...
    trigger_mod.TriggerRule = types.SimpleNamespace(
    ...
    module_path = (
        Path(__file__).resolve().parents[2] / "airflow" / "dags" / "galaxy_pipeline.py"
    )
...
def test_dvc_lock_sha256_and_provenance_keys(galaxy_pipeline, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        galaxy_pipeline, "FEEDBACK_TRAINING_MANIFEST_PATH", tmp_path / "feedback.csv"
    )
    monkeypatch.setattr(
        galaxy_pipeline, "_latest_mlflow_run_id", lambda: "mlflow-run-1"
    )
    monkeypatch.setattr(
        galaxy_pipeline,
        "_control_plane_state",
        lambda: {"last_feedback_snapshot_at": "snapshot-1"},
    )
    monkeypatch.setattr(
        galaxy_pipeline,
        "load_pipeline_artifact",
        lambda *args, **kwargs: {
            "manifest_path": "artifacts/feedback_training_manifest.csv"
        },
    )

    provenance = galaxy_pipeline.save_dvc_provenance(
        run_id="scheduled__2026-04-28T00:00:00+00:00"
    )
    ...
    assert (
        provenance["dvc_lock_sha256"]
        == hashlib.sha256(dvc_lock.read_bytes()).hexdigest()
    )
...
def test_dvc_push_obeys_true_false_config(galaxy_pipeline, monkeypatch):
    ...
    monkeypatch.setattr(
        galaxy_pipeline,
        "_run_command",
        lambda command, use_training_venv=False: calls.append(command),
    )
...
def test_mlflow_logging_failure_raises(galaxy_pipeline, tmp_path, monkeypatch):
    ...
    (run_dir / "provenance.json").write_text(
        json.dumps({"mlflow_run_id": "run-1"}), encoding="utf-8"
    )
    ...
    broken_mlflow = types.SimpleNamespace(
        ...
        log_artifact=lambda *args, **kwargs: (_ for _ in ()).throw(
            RuntimeError("mlflow artifact failed")
        ),
    )
...
def test_validation_accepts_and_rejects_thresholds(galaxy_pipeline, monkeypatch):
    monkeypatch.setattr(
        ...
        "_required_config",
        lambda key: {
            "continuous_improvement.accuracy_threshold": 0.85,
            "continuous_improvement.macro_f1_threshold": 0.8,
        }[key],
    )
    ...
    monkeypatch.setattr(
        galaxy_pipeline,
        "_candidate_metrics",
        lambda: {"accuracy": 0.9, "macro_f1": 0.81},
    )
    ...
    monkeypatch.setattr(
        galaxy_pipeline,
        "_candidate_metrics",
        lambda: {"accuracy": 0.9, "macro_f1": 0.79},
    )
